...
bool motorPermanentFault = false;  // IMP-07: set after repeated overloads, requires power cycle
...
float liftRightLP = 0;
bool bumper = false;
bool bumperLeft = false;   // TASK-BUMPER-01: PA4 (bumperX channel) = left bumper segment
bool bumperRight = false;  // TASK-BUMPER-01: PA5 (bumperY channel) = right bumper segment
bool ovCheck = false;
...
void readSensors(){
  ...
  // lift (IMP-09: pinLift1→liftRight and pinLift2→liftLeft is intentional per PCB wiring)
  liftRight = analogRead(pinLift1);
  liftLeft  = analogRead(pinLift2);
  
  // bumper — separate left/right channels (TASK-BUMPER-01)
  // PA4 (bumperX) = left segment, PA5 (bumperY) = right segment; resting centre ~300, threshold 150
  bumperX    = analogRead(pinBumperX);
  bumperY    = analogRead(pinBumperY);
  bumperLeft  = (abs(bumperX-300) > 150);
  bumperRight = (abs(bumperY-300) > 150);
  bumper      = (bumperLeft || bumperRight);  // combined, kept for backward compat

  // lift low-pass filtering
  w = 0.5;
  liftLeftLP = w * liftLeftLP + (1.0-w) * ((float)liftLeft);
  liftRightLP = w * liftRightLP + (1.0-w) * ((float)liftRight);
  if ((abs(liftLeft-530) < 200) && (abs(liftRight-530) < 200)){  // IMP-03 fix: AND — reset LP only when both sensors are in normal position
    liftLeftLP = liftLeft;
  ...
  if (bumperLeft)  CONSOLE.println("BUMPER LEFT");
  if (bumperRight) CONSOLE.println("BUMPER RIGHT");
  if (motorMowFault){
...
void readMotorCurrent(){      
  ...
  w = 0.95;    // IMP-05 fix: faster response ~200ms (was 0.99 ~1s) for mow motor overload detection
  mowCurrLP = w * mowCurrLP + (1.0-w) * mowCurr;

  if ((mowCurrLP > 4.0) || (motorLeftCurrLP > 1.5) || (motorRightCurrLP > 1.5)) {
    // IMP-07: escalate recovery timeout on repeated overloads within 60s window
    if (millis() > motorOverloadWindowStart + 60000) {
      motorOverloadCount = 0;
      motorOverloadWindowStart = millis();
    }
    motorOverloadCount++;
    if (motorOverloadCount > 5) {
      motorPermanentFault = true;  // requires power cycle to clear
    }
    unsigned long recoveryMs = (motorOverloadCount <= 3) ? 2000UL : 10000UL;
    motorOverload = true;
    motorOverloadTimeout = millis() + recoveryMs;
  }

  // charging current (reading charging current does not work for some reason...)
  // maximum charging current is 1.7 amps
  chgCurrent = ((float)analogRead(pinChargeI)) * 1.0;  // BUG-09: calibration factor unknown; raw ADC value, not Ampere
  w = 0.99;
...
void setup() {
  ...
  IWatchdog.begin(2000000); // IMP-06 fix: 2s (was 6s) — safer for outdoor robot
}
...
void cmdMotor(){
  ...
  for (int idx=0; idx < cmd.length(); idx++){
    ...
    if ((ch == ',') || (idx == cmd.length()-1)){
      ...
      // BUG-07: Pi sends AT+M,rightPwm,leftPwm — field1 maps to left, field2 to right (intentional cross-wiring compensation)
      if (counter == 1){
          left = intValue;
      } else if (counter == 2){
          right = intValue;
...
void cmdSummary(){
  String s = F("S,");
  s += batVoltageLP;  // IMP-04 fix: LP-filtered value (was raw batVoltage), avoids ADC spike triggering shutdown
  s += ",";
  s += chgVoltage;
  s += ",";
  s += chgCurrentLP;
  ...
  s += int(raining);
  s += ",";
  s += int(motorMowFault || motorOverload || motorPermanentFault);  // IMP-02 fix: hardware fault + software overload + permanent fault combined
  s += ",";
  s += mowCurrLP;
  s += ",";
  s += motorLeftCurrLP;
...
void loop() {
  ...
  if (millis() > motorOverloadTimeout){
    if (!motorPermanentFault) motorOverload = false;  // IMP-07: permanent fault blocks recovery
  }
