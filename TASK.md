...
| 2026-04-02 | Phase 3.1 off-matrix: STM flash maintenance window prevents false MCU comm fault | Done | `WebSocketServer` now marks a controlled STM flash maintenance window before invoking the uploaded-binary flash helper, `Robot` suppresses transient MCU comm-loss escalation during that window plus a short recovery grace period, and a successful WebUI flash now schedules an automatic `sunray-core.service` restart so Alfred comes back on clean telemetry instead of latching `ERR_MCU_COMMS` first. Focused `[run][safety]` and backend build green; full-suite verification rerun in progress. | Deploy to Alfred and confirm the next STM WebUI flash no longer enters `Error` before the automatic restart brings the runtime back. |
| 2026-04-02 | Phase 3.1 off-matrix: full Alfred WebUI STM flashing loop field-proven | Done | Alfred now proves the staged STM flow end-to-end: SWD probe over Pi GPIO, uploaded `.bin` flash from the WebUI, post-flash comm-loss tolerance, automatic `sunray-core.service` restart, and version re-observation over UART. Follow-up firmware hotfixes `RM18 v1.1.22` (detailed mow-fault cause telemetry) and `RM18 v1.1.23` (`OV-Check` gated to active mower operation) were also validated through that same path. | Shift future STM work from “can we flash remotely?” to “how do we harden/clean up the firmware and recovery UX?” |
