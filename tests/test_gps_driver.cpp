...
namespace sunray
{
    ...
    TEST_CASE("MockGpsDriver: getData returns injected data", "[gps]")
    {
        ...
        drv.injected.numCorrSignals = 11;
    ...
    TEST_CASE("GpsData default state is invalid", "[gps]")
    {
        ...
        CHECK(d.numSV == 0);
        CHECK(d.numCorrSignals == 0);
        CHECK(d.nmeaGGA.empty());
    }

    TEST_CASE("Corrected signal count is exposed", "[gps]")
    {
        MockGpsDriver drv;
        drv.injected.numCorrSignals = 9;
        drv.injected.valid = true;

        const auto d = drv.getData();
        CHECK(d.numCorrSignals == 9);
    }
