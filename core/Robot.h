...
namespace sunray
{
    ...
    public:
        ...
        // N6.3: resume after docking mid-mission
        void requestResumeActiveMission();

        /// N6.1: cache the plan that was built for the most-recent preview.
        /// Thread-safe — called from the WebSocket/Crow thread.
        void storePendingMissionPlan(nav::MissionPlan plan);
        ...
        nav::RuntimeState &runtimeState() { return runtimeState_; }
        const nav::RuntimeState &runtimeState() const { return runtimeState_; }
    ...
    private:
        ...
        void activateMissionPlan(const nav::MissionPlan &plan);
        nav::MissionPlan buildOperatorMissionPlan(const std::vector<std::string> &zoneIds = {}) const;
        void resumeActiveMission(); // N6.3
        void buildAndStartWaypointPlan(const nav::MissionPlan &plan);
        void refreshActiveMissionProgress();
        bool canStartMowingMission();
        bool canStartPlannedMission(const nav::MissionPlan &plan);
        bool canStartPlannedMission(const nav::RoutePlan &route);
        int activeMissionZoneIndex() const;
        ...
        nav::RuntimeState runtimeState_;         ///< active runtime state and replanning
        nav::LineTracker lineTracker_;           ///< Stanley controller
        nav::WaypointExecutor waypointExecutor_; ///< N2.5: new-architecture waypoint plan executor
        ...
        nav::MissionPlan activeMissionPlan_;
        nav::RuntimeSnapshot runtimeSnapshot_;

        // ── N6: Plan lifecycle ────────────────────────────────────────────────
        // N6.1: pending plan cached from the latest planner-preview response.
        // Set from WebSocket/Crow thread; consumed on the Robot run() thread.
        mutable std::mutex pendingPlanMutex_;
        nav::MissionPlan pendingMissionPlan_;
        bool hasPendingMissionPlan_ = false;

        // N6.3: plan saved when docking interrupts an active mission.
        nav::MissionPlan interruptedMissionPlan_;
        int interruptedMowPointsIdx_ = -1;
        bool hasInterruptedMission_ = false;
