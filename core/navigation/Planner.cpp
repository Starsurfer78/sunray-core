...
RoutePlan Planner::planPath(const Map& map, const PlannerContext& context) {
    ...
    if (!map.pathAllowed(candidatePath)) return {};
    ...
    for (const auto& waypoint : path) {
        ...
        routePoint.slow = (context.planningMode == WayType::DOCK)
            && (waypoint.distanceTo(context.destination) <= map.dockMeta_.slowZoneRadius_m);
