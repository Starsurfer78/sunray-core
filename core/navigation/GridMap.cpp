...
namespace sunray
{
    namespace nav
    {
        ...
        bool GridMap::lineIsClear(const Point &a, const Point &b) const
        {
            ...
            for (int i = 0; i <= n; ++i)
            {
                ...
                if (!worldToGrid(px, py, gx, gy))
                    return false;
                if (cellAt(gx, gy) == Costmap::Cell::OCCUPIED)
                    return false;
            }
            return true;
        }
        ...
        std::vector<Point> GridMap::planPath(const Point &src, const Point &dst) const
        {
            ...
            int sx, sy, dx, dy;
            if (!worldToGrid(src.x, src.y, sx, sy))
                return {};

            // Destination must be reachable as requested. Do not silently redirect to a
            // nearby free cell — callers must see the endpoint as unplannable.
            ...
            while (!openSet.empty())
            {
                ...
                for (int d = 0; d < 8; ++d)
                {
                    ...
                    if (closed[nidx])
                        continue;
                    if (cellAt(nx, ny) == Costmap::Cell::OCCUPIED)
                        continue;
                    if ((DDX[d] != 0) && (DDY[d] != 0))
                    {
            ...
            // Replace last point with exact dst once we know the exact destination cell
