...
namespace sunray
{
    ...
    bool HistoryDatabase::appendEvent(const EventRecord &event, Logger &logger)
    {
        ...
        if (event.batteryV.has_value())
            sqlite3_bind_double(stmt, 10, *event.batteryV);
        else
            sqlite3_bind_null(stmt, 10);

        if (event.gpsSol.has_value())
            sqlite3_bind_int(stmt, 11, *event.gpsSol);
        else
            sqlite3_bind_null(stmt, 11);

        if (event.x.has_value())
            sqlite3_bind_double(stmt, 12, *event.x);
        else
            sqlite3_bind_null(stmt, 12);

        if (event.y.has_value())
            sqlite3_bind_double(stmt, 13, *event.y);
        else
    ...
    bool HistoryDatabase::upsertSession(const SessionRecord &session, Logger &logger)
    {
        ...
        if (session.endedAtMs > 0)
            sqlite3_bind_int64(stmt, 3, static_cast<sqlite3_int64>(session.endedAtMs));
        else
    ...
    nlohmann::json HistoryDatabase::listEvents(unsigned limit, Logger &logger) const
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (sqlite3_column_type(stmt, 9) != SQLITE_NULL)
                row["battery_v"] = sqlite3_column_double(stmt, 9);
            if (sqlite3_column_type(stmt, 10) != SQLITE_NULL)
                row["gps_sol"] = sqlite3_column_int(stmt, 10);
            if (sqlite3_column_type(stmt, 11) != SQLITE_NULL)
                row["x"] = sqlite3_column_double(stmt, 11);
            if (sqlite3_column_type(stmt, 12) != SQLITE_NULL)
                row["y"] = sqlite3_column_double(stmt, 12);
    ...
    nlohmann::json HistoryDatabase::listSessions(unsigned limit, Logger &logger) const
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (sqlite3_column_type(stmt, 2) != SQLITE_NULL)
                row["ended_at_ms"] = static_cast<long long>(sqlite3_column_int64(stmt, 2));
            if (sqlite3_column_type(stmt, 8) != SQLITE_NULL)
                row["mean_gps_accuracy_m"] = sqlite3_column_double(stmt, 8);
            if (sqlite3_column_type(stmt, 9) != SQLITE_NULL)
                row["max_gps_accuracy_m"] = sqlite3_column_double(stmt, 9);
            if (sqlite3_column_type(stmt, 10) != SQLITE_NULL)
            {
    ...
    nlohmann::json HistoryDatabase::buildSummary(Logger &logger) const
    {
        ...
        auto queryScalar = [&](const char *sql, const char *key, bool integer = true)
        {
            ...
            {
                if (integer)
                    result[key] = static_cast<long long>(sqlite3_column_int64(stmt, 0));
                else
                    result[key] = sqlite3_column_double(stmt, 0);
            }
            sqlite3_finalize(stmt);
        };
    ...
    bool HistoryDatabase::clear(Logger &logger)
    {
        std::lock_guard<std::mutex> lk(mutex_);

        if (!enabled_ || !available_)
        {
            logger.warn("HistoryDatabase", "clear() called but history database disabled or unavailable");
            return false;
        }
        ...
        // Clear both events and sessions tables
        const char *deleteEventsSql = "DELETE FROM events;";
        const char *deleteSessionsSql = "DELETE FROM sessions;";

        char *errMsg = nullptr;

        int rc = sqlite3_exec(db, deleteEventsSql, nullptr, nullptr, &errMsg);
        if (rc != SQLITE_OK)
        {
            logger.error("HistoryDatabase", "failed to clear events table: " + std::string(errMsg));
            sqlite3_free(errMsg);
            sqlite3_close(db);
            return false;
        }

        rc = sqlite3_exec(db, deleteSessionsSql, nullptr, nullptr, &errMsg);
        if (rc != SQLITE_OK)
        {
            logger.error("HistoryDatabase", "failed to clear sessions table: " + std::string(errMsg));
            sqlite3_free(errMsg);
            sqlite3_close(db);
            return false;
        }
        ...
        sqlite3_close(db);
        logger.info("HistoryDatabase", "history database cleared successfully");
        return true;
#endif
    }
    ...
    bool HistoryDatabase::initSqlite(const Config &, Logger &logger)
    {
        ...
        if (!schemaVersion.has_value() && std::filesystem::exists(path_) && std::filesystem::file_size(path_, ec) > 0)
        {
            if (!backupExistingDatabase(logger))
            {
                sqlite3_close(db);
    ...
    bool HistoryDatabase::applyRetention(void *dbHandle, Logger &logger) const
    {
        ...
        if (maxEvents_ > 0)
        {
            const std::string sql =
                ...
                "SELECT id FROM events ORDER BY ts_ms DESC LIMIT " +
                std::to_string(maxEvents_) +
                ");";
            if (!runDelete(sql, "events"))
                return false;
        }
        ...
        if (maxSessions_ > 0)
        {
            const std::string sql =
                ...
                "SELECT id FROM sessions ORDER BY started_at_ms DESC LIMIT " +
                std::to_string(maxSessions_) +
                ");";
            if (!runDelete(sql, "sessions"))
                return false;
        }
        return true;
#endif
