...
namespace sunray
{
    ...
    void EscapeReverseOp::begin(OpContext &ctx)
    {
        ...
        if (ctx.map)
            ctx.map->addObstacle(ctx.x, ctx.y, ctx.now_ms);
    }
    ...
    void EscapeReverseOp::run(OpContext &ctx)
    {
        ...
        if (hitLeft && !hitRight)
            escapeAngular = -0.15f; // hit left → steer right
        else if (hitRight && !hitLeft)
            escapeAngular = 0.15f; // hit right → steer left
        ...
        if (ctx.now_ms >= driveReverseStopTime_ms)
        {
            ...
            if (nextOp != nullptr)
            {
                ...
                if (ctx.map && ctx.runtimeState &&
                    (ctx.runtimeState->wayMode() == nav::WayType::MOW || ctx.runtimeState->wayMode() == nav::WayType::DOCK))
                {
                    const bool replanned = ctx.runtimeState->replanToCurrentTarget(*ctx.map, ctx.x, ctx.y);
                    if (replanned)
                    {
