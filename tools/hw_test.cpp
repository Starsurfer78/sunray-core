...
static void rep(const std::string &name, Result r, const std::string &msg)
{
    ...
    const char *tag = (r == Result::PASS) ? "\033[32m[PASS]\033[0m" : (r == Result::FAIL) ? "\033[31m[FAIL]\033[0m"
...
static bool verifyCrc(std::string &frame)
{
    ...
    for (size_t i = 0; i < idx; i++)
        expected += static_cast<uint8_t>(frame[i]);
...
private:
    ...
    std::string drainCrcFrame(char frameType)
    {
        ...
        if (crcPos == std::string::npos || crcPos + 5 > rxBuf_.size())
            return "";
...
static void sendMotorPwm(AtLink &at, int left, int right, int mow)
{
    ...
    at.sendRaw("AT+M," + std::to_string(right) + "," + std::to_string(left) + "," + std::to_string(mow));
}
...
static MotorFrame parseMotorFrame(const std::string &frame)
{
    ...
    try
    {
        ...
        if (f.size() > 1)
            m.ticksLeft = std::stoul(f[1]);
        if (f.size() > 2)
            m.ticksRight = std::stoul(f[2]);
        if (f.size() > 3)
            m.ticksMow = std::stoul(f[3]);
        if (f.size() > 7)
            m.stopButton = (std::stoi(f[7]) != 0);
        if (f.size() > 8)
            m.fault = (std::stoi(f[8]) != 0);
        m.valid = true;
    }
    catch (...)
    {
...
static bool phase1Uart(sunray::platform::Serial &uart)
{
    ...
    }
    {
        ...
        try
        {
            ...
            if (batT > -9000.0f)
                ss << "  batT=" << fmtFloat(batT) << " °C";
            if (rain)
                ss << "  \033[33mREGEN\033[0m";
                      ...
                      sunray::platform::PortExpander &ex3)
{
    ...
    (void)ex2;
    (void)ex3; // werden in Phase 3 benutzt
}
                      ...
                      const std::function<bool(const std::vector<std::string> &)> &check)
{
    ...
    while (nowMs() < deadline)
    {
        ...
        if (!f.empty())
        {
            ...
            if (check(fields))
                return static_cast<int>(nowMs() - start);
        }
    }
    return -1;
}
        ...
        else
        {
            ...
            else
            {
                ...
                const int ms = pollSensor(at, kPollMs,
                                          [](const auto &f)
                                          { return f.size() > 7 && std::stoi(f[7]) != 0; });
                ...
                else
                {
                    ...
                    rep("E-Stop", (msRel >= 0) ? Result::PASS : Result::WARN,
                        "erkannt nach " + std::to_string(ms) + " ms" + (msRel >= 0 ? ", rückgestellt" : " — Rückstellung nicht erkannt"));
                }
            }
        }
    }
...
static MotorCurrents readCurrents(AtLink &at, int leftPwm, int rightPwm, int mowPwm)
{
    ...
    try
    {
        if (f.size() > 8)
            c.mowA = std::stof(f[8]);
        if (f.size() > 9)
            c.leftA = std::stof(f[9]);
        if (f.size() > 10)
            c.rightA = std::stof(f[10]);
        c.valid = true;
    }
    catch (...)
    {
...
static void motorRampTest(AtLink &at)
{
    ...
    while (nowMs() < holdEnd)
    {
        ...
        else
        {
            ...
            if (!f.empty())
            {
                ...
                if (m.valid)
                {
                    ticksLend = m.ticksLeft;
                    ticksRend = m.ticksRight;
                }
                if (m.valid && m.fault)
                {
                    sendMotorPwm(at, 0, 0, 0);
    ...
    {
        std::string msg = "ΔL=" + std::to_string(dL) + "  ΔR=" + std::to_string(dR) + "  maxPWM=" + std::to_string(kMotorMaxPwm);
        Result r = Result::PASS;
        if (dL < 10 && dR < 10)
        {
            msg += " — kaum Ticks, Encoder prüfen";
            r = Result::WARN;
        }
        else if (dL < 5 || dR < 5)
        {
            msg += " — ein Encoder liefert zu wenig Ticks";
            r = Result::WARN;
        }
    ...
    if (currents.valid)
    {
        const std::string msgA = "links=" + fmtFloat(currents.leftA) + " A" + "  rechts=" + fmtFloat(currents.rightA) + " A" + "  (PWM=" + std::to_string(kMotorMaxPwm) + ")";
...
static void mowMotorRampTest(AtLink &at)
{
    ...
    {
        std::string msg = "ΔMow=" + std::to_string(dM) + "  maxPWM=" + std::to_string(kMowMaxPwm);
    ...
    if (mowCurrents.valid)
    {
        const std::string msgA = "mow=" + fmtFloat(mowCurrents.mowA) + " A" + "  (PWM=" + std::to_string(kMowMaxPwm) + ")";
        Result rA = Result::PASS;
...
static void phase7Gps()
{
    ...
    if (hasUbx)
    {
        ...
        std::cout << "  Info: NMEA-Ausgabe im F9P deaktiviert oder UBX bereits aktiv."
                  << " Hardware OK.\n";
        return;
    }
    ...
    if (!hasGga)
        rep("GPS Daten", Result::WARN,
            std::to_string(rawBytes) + " Bytes / " + std::to_string(nmeaLines.size()) + " NMEA-Sätze — kein GGA");
    else if (!hasFix)
...
static void printSummary()
{
    ...
    for (const auto &r : gReports)
    {
        ...
        case Result::FAIL:
            fail++;
            failures.push_back(r.name);
            break;
        case Result::WARN:
            warn++;
            warnings.push_back(r.name);
            break;
        case Result::SKIP:
            skip++;
            break;
...
int main(int argc, char **argv)
{
    ...
    {
        if (std::string(argv[i]) == "--skip-motors")
            skipMotors = true;
    }
