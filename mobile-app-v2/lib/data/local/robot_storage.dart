...
class RobotStorage {
  ...
  Future<List<SavedRobot>> loadSavedRobots() async {
    ...
    if (robotsJson is! List<dynamic>) {
      return const <SavedRobot>[];
    }

    return robotsJson
        .whereType<Map<String, dynamic>>()
        .map(_savedRobotFromJson)
        .where((robot) => robot.id.isNotEmpty && robot.lastHost.isNotEmpty)
  ...
  Future<SavedRobot?> loadActiveRobot() async {
    ...
    if (activeRobotId is! String || activeRobotId.isEmpty) {
      return null;
    }
  ...
  Future<void> forgetRobot(String robotId) async {
    final state = await _readState();
    final robots = await loadSavedRobots();
    final updated = robots
        .where((entry) => entry.id != robotId)
        .toList(growable: false);
    final activeRobotId = state['activeRobotId'] == robotId
        ? null
        : state['activeRobotId'];
  ...
  Future<Map<String, dynamic>> _readState() async {
    try {
      ...
      // Keep startup resilient if local storage is missing or unavailable.
    }
